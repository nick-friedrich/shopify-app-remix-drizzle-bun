...
4. **Docker**: [Download and install](https://www.docker.com/products/docker-desktop) it if you haven't already.
...
Then, start the database using Docker Compose:
...
This template uses [Drizzle ORM](https://orm.drizzle.team/) to store session data, by default using a [PostgreSQL](https://www.postgresql.org/) database.
The database schema is defined in `app/db.server.ts`.
